...
import ListItem from '~/components/operator/tables/TablesListItem.vue';
import ListItemSkeleton from '~/components/operator/tables/TablesListItemSkeleton.vue';
import SearchForm from '~/components/operator/tables/TablesSearchForm.vue';
...
async function onSearchSubmit(form: Partial<ITable>) {
      ...
      <div class="container">
        <div class="page-sub-title__inner">
          <h2 class="page-sub-title__title">
            <el-icon size="18"><Files /></el-icon> Список столов
          </h2>
          <div class="page-sub-title__actions">
            <el-button type="primary" :icon="Plus" @click="router.push('/tables/create')"
              >Добавить</el-button
            >
          </div>
        </div>
      ...
      <div class="row" v-if="items.length">
        <div class="col-12 col-md-6 mt-4" v-for="item in items">
          <ListItem
            ...
            :item="item"
            @delete="onDeleteItem"
            @click="router.push(`/tables/detail/${item.id}`)"
