...
import ListItem from '~/components/operator/clients/ClientsListItem.vue';
import ListItemSkeleton from '~/components/operator/clients/ClientsListItemSkeleton.vue';
import SearchForm from '~/components/operator/clients/ClientsSearchForm.vue';
...
let searchFormData: Partial<IClient> = {};
let page = 0;
let limit = 10;
...
async function onSearchSubmit(form: Partial<IClient>) {
      ...
      <div class="container">
        <div class="page-sub-title__inner">
          <h2 class="page-sub-title__title">
            <el-icon size="18"><Files /></el-icon> Список клиентов
          </h2>
          <div class="page-sub-title__actions">
            <el-button type="primary" :icon="Plus" @click="router.push('/clients/create')"
              >Добавить</el-button
            >
          </div>
        </div>
      ...
      <div class="row" v-if="items.length">
        <div class="col-12 col-md-6 mt-4" v-for="item in items">
          <ListItem
            ...
            :item="item"
            @delete="onDeleteItem"
            @click="router.push(`/clients/detail/${item.id}`)"
